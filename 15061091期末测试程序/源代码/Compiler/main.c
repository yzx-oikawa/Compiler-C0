...
FILE *IN, *OUT, *INTER, *ASSEM, *ERROR, *NEWINTER, *NEWASSEM;
...
void newinter2assem();//优化后的四元式转汇编
...
void partition()
{
    ...
    for(interi=1;interi<=interpc;interi++)
    {
        ...
        if(strcmp(intercode[interi].inter0,"assign")==0&&
           strcmp(intercode[interi].inter1,"ret")!=0)
        {
            //先判断赋值变量b的变量名是否在节点表中
        ...
        //四则运算z=x+y;
                ...
                &&((strcmp(intercode[interi+1].inter0,"assign")!=0)
                ||(strcmp(intercode[interi+1].inter0,"assign")==0&&
                      strcmp(intercode[interi].inter1,intercode[interi+1].inter3)!=0&&
                      strcmp(intercode[interi].inter2,intercode[interi+1].inter3)!=0)))
        {
            //判断操作数x的变量名是否在节点表中
            for(listi=0;listi<=listpc;listi++)
            {
...
int differ()
{
    ...
    while(1){
        ...
        if(sym==voidsym||sym==rbrace||sym==lbrace||sym==ifsym||sym==whilesym
           ||sym==scanfsym||sym==printfsym||sym==switchsym
    ...
    while(!feof(IN)){
        switch(sym)
        {
            ...
            case semicolon:
                ...
                if(sym==rbrace)//右花括号
                    return;
                else if(sym==intsym||sym==charsym||sym==voidsym)
                    return;
                else continue;
            case ifsym:
                ifstatement();
    ...
    while(1)
    {
        ...
        if(sym==sinquo||sym==inttype||sym==numtype||sym==zero||sym==add||sym==sub)//单引号括起来的字母或数字，或者整数
        {
            ...
            if(constflag>0){//常量
                ...
                printerror(1);
                while(sym!=casesym&&sym!=defaultsym&&sym!=lbrace)
                    sym=nextsym();
                continue;
            }
            if(constflag==2){//char
            ...
            printerror(1);
            while(sym!=casesym&&sym!=defaultsym&&sym!=rbrace)
                sym=nextsym();
            continue;
        }
    }
    ...
    if(sym!=lparent)//左括号
        ...
        while(1)
        {
            if(sym==identsym||sym==chartype)//标识符
            {
                ...
                printerror(1);
                while(sym!=identsym&&sym!=chartype&&sym!=rparent)
                    sym=nextsym();
                if(sym==rparent){
                    sym=nextsym();//分号
                    if(sym!=semicolon)
...
void newinter2assem()//优化后的四元式转汇编
{
    ...
    int functab; //函数在符号表中的位置
    char funcname[MAXIDENTLEN];
    fprintf(NEWASSEM,".kdata\n");
    fprintf(NEWASSEM,"\tenter: .ASCIIZ \"\\n\"\n");
    for(newi=1;newi<=newpc;newi++)
    {
        if(strcmp(newinter[newi].inter0,"print")==0){
            if(newinter[newi].inter1[0]!='\0')
            {
                if(mipstrpc==0)
                {
                    itoa(stri++,strtemp,10);
                    strcpy(mipstring[mipstrpc].name,"str");
                    strcat(mipstring[mipstrpc].name,strtemp);
                    strcpy(mipstring[mipstrpc++].str,newinter[newi].inter1);
                }
                for(mipstri=0;mipstri<mipstrpc;mipstri++)
                {
                    if(strcmp(newinter[newi].inter1,mipstring[mipstri].str)==0)
                        break;
                    if(mipstri==mipstrpc-1)//未保存的字符串
                    {
                        itoa(stri++,strtemp,10);
    ...
    for(mipstri=0;mipstri<mipstrpc;mipstri++)
    {
        fprintf(NEWASSEM,"\t%s: .ASCIIZ \"%s\"\n",mipstring[mipstri].name,mipstring[mipstri].str);
    }
    fprintf(NEWASSEM,".text\n");
    fprintf(NEWASSEM,"\tli $fp, 0x00000000\n");
    fprintf(NEWASSEM,"\tli $sp, 0x00000000\n");
    for(newi=1;newi<=newpc;newi++)
    {
        memset(token, 0, sizeof(token));
        //常量定义
        if(strcmp(newinter[newi].inter0,"const")==0)
        {
            strcpy(token,newinter[newi].inter2);
            //全局常量定义
            if(checkiflog()==0&&symtab[tabi].location==0)
            {
                fprintf(NEWASSEM,"\tli $s0,%d\n",symtab[tabi].value);
                fprintf(NEWASSEM,"\tsw $s0,0($fp)\n");
                //移动fp
                fprintf(NEWASSEM,"\taddi $fp,$fp,4\n");
                //移动sp
                fprintf(NEWASSEM,"\tor $sp,$0,$fp\n");
            }
            //局部常量定义
            if(checkiflog()==0&&symtab[tabi].location>0)
            {
                fprintf(NEWASSEM,"\tli $s0,%d\n",symtab[tabi].value);
                fprintf(NEWASSEM,"\tsw $s0,0($sp)\n");
                //移动sp
                fprintf(NEWASSEM,"\taddi $sp,$sp,4\n");
            }

        }
        //变量定义
        else if(strcmp(newinter[newi].inter0,"var")==0)
        {
            strcpy(token,newinter[newi].inter2);
            //全局变量定义
            if(checkiflog()==1&&symtab[tabi].location==0)
            ...
            if(checkiflog()==1&&symtab[tabi].location>0)
            {
                fprintf(NEWASSEM,"\tli $s0,0\n");
                fprintf(NEWASSEM,"\tsw $s0,0($sp)\n");
                //移动sp
                fprintf(NEWASSEM,"\taddi $sp,$sp,%d\n",symtab[tabi].length==0?4:symtab[tabi].length*4);
            }
        }
        //函数定义
        else if(strcmp(newinter[newi].inter0,"func")==0)
        {
            if(gotomainflag==0)
            {
                fprintf(NEWASSEM,"\taddi $sp,$fp,4\n");
                fprintf(NEWASSEM,"\tj Label_main\n");
                gotomainflag=1;
            }
            symloca++;
            strcpy(token,newinter[newi].inter2);
            //有返回值函数定义
            if(strcmp(newinter[newi].inter1,"int")==0
            ||strcmp(newinter[newi].inter1,"char")==0)
            {
                if(checkiflog()==3){
                    defloc=symtab[tabi].location;
                    fprintf(NEWASSEM,"Label_%s:\n",token);
                    fprintf(NEWASSEM,"\tsw $ra,4($sp)\n",token);
                    fprintf(NEWASSEM,"\taddi $fp,$sp,16\n");
                    fprintf(NEWASSEM,"\taddi $sp,$fp,4\n");
                    //留出tn的位置
                    fprintf(NEWASSEM,"\taddi $sp,$sp,%d\n",4*tncount[defloc]);
                }
            ...
            //无返回值函数定义
            else if(strcmp(newinter[newi].inter1,"void")==0)
            {
                if(checkiflog()==2){
                    defloc=symtab[tabi].location;
                    fprintf(NEWASSEM,"Label_%s:\n",token);
        ...
        //参数定义
        else if(strcmp(newinter[newi].inter0,"para")==0)
        {
            strcpy(token,newinter[newi].inter2);
            if(checkiflog()==4)
                //移动sp
        ...
        //赋值语句
        else if(strcmp(newinter[newi].inter0,"assign")==0)
        {
            //被赋的值
            strcpy(token,newinter[newi].inter1);
            if(strcmp(token,"ret")==0)//返回值
            {
                fprintf(NEWASSEM,"\tlw $s0,8($sp)\n");
            }
            else if(checkiflog()==0||checkiflog()==1||checkiflog()==4)//常量或变量或参数
            {
                if(symtab[tabi].location==0)//全局常变量
                    fprintf(NEWASSEM,"\tlw $s0,%d($0)\n",symtab[tabi].address);
                else//局部常变量
                    fprintf(NEWASSEM,"\tlw $s0,%d($fp)\n",4*tncount[defloc]+symtab[tabi].address);
            }
            else if(checkiftn()>0)//tn
                fprintf(NEWASSEM,"\tlw $s0,%d($fp)\n",checkiftn()*4);
            else//立即数
                fprintf(NEWASSEM,"\tori $s0,$0,%s\n",token);

            //取相反数
            if(newinter[newi].inter2[0]=='-')
            {
                fprintf(NEWASSEM,"\tsub $s0,$0,$s0\n");
            }

            //赋给的值
            strcpy(token,newinter[newi].inter3);
            if(checkiflog()==1||checkiflog()==4)//变量或参数
            {
                if(symtab[tabi].location==0)//全局变量
                    fprintf(NEWASSEM,"\tsw $s0,%d($0)\n",symtab[tabi].address);
                else //局部变量
                    fprintf(NEWASSEM,"\tsw $s0,%d($fp)\n",4*tncount[defloc]+symtab[tabi].address);
            }
            else if(checkiftn()>0)//tn
                fprintf(NEWASSEM,"\tsw $s0,%d($fp)\n",checkiftn()*4);
        }
        //给数组赋值
        else if(strcmp(newinter[newi].inter0,"arrass")==0)
        {
            //被赋的值
            strcpy(token,newinter[newi].inter1);
            if(strcmp(token,"ret")==0)//返回值
            ...
            if(checkiflog()==1)
            {
                ...
                if(symtab[tabi].location==0)//全局变量
                {
                    ...
                    {
                        if(symtab[tabi].location==0)//全局常变量
                            fprintf(NEWASSEM,"\tlw $s1,%d($0)\n",symtab[tabi].address);
                        else//局部常变量
                            fprintf(NEWASSEM,"\tlw $s1,%d($fp)\n",4*tncount[defloc]+symtab[tabi].address);
                    }
                    else if(checkiftn()>0)//tn
                        fprintf(NEWASSEM,"\tlw $s1,%d($fp)\n",checkiftn()*4);
                    else//立即数
                        fprintf(NEWASSEM,"\tori $s1,$0,%s\n",token);
                    fprintf(NEWASSEM,"\tmul $s1,$s1,4\n");
                    fprintf(NEWASSEM,"\taddi $s1,$s1,%d\n",arrayaddr);
                    fprintf(NEWASSEM,"\tsw $s0,0($s1)\n");
                }
                else//局部变量
                {
                    arrayaddr=symtab[tabi].address;
        ...
        //取数组的值
        else if(strcmp(newinter[newi].inter0,"arrget")==0)
        {
            //被赋的值
            strcpy(token,newinter[newi].inter1);
            if(checkiflog()==1)
        ...
        //加减乘除
        else if(strcmp(newinter[newi].inter0,"+")==0||strcmp(newinter[newi].inter0,"-")==0||
                strcmp(newinter[newi].inter0,"*")==0||strcmp(newinter[newi].inter0,"/")==0)
        {
            if(strcmp(newinter[newi].inter0,"+")==0) op=0;
            else if(strcmp(newinter[newi].inter0,"-")==0) op=1;
            else if(strcmp(newinter[newi].inter0,"*")==0) op=2;
            else if(strcmp(newinter[newi].inter0,"/")==0) op=3;
            //第一个操作数
            strcpy(token,newinter[newi].inter1);
            if(checkiflog()==0||checkiflog()==1||checkiflog()==4)//常量或变量或参数
            {
            ...
            {
                if(symtab[tabi].location==0)//全局常变量
                    fprintf(NEWASSEM,"\tlw $s2,%d($0)\n",symtab[tabi].address);
                else//局部常变量
                    fprintf(NEWASSEM,"\tlw $s2,%d($fp)\n",4*tncount[defloc]+symtab[tabi].address);
            }
            else if(checkiftn()>0)//tn
                fprintf(NEWASSEM,"\tlw $s2,%d($fp)\n",checkiftn()*4);
            else//立即数
                fprintf(NEWASSEM,"\tori $s2,$0,%s\n",token);

            //加减乘除操作
            //fprintf(NEWASSEM,"%d\n",op);
            if(op==0)//加法
                fprintf(NEWASSEM,"\tadd $s0,$s1,$s2\n");
            else if(op==1)//减法
                fprintf(NEWASSEM,"\tsub $s0,$s1,$s2\n");
            else if(op==2)//乘法
                fprintf(NEWASSEM,"\tmul $s0,$s1,$s2\n");
            else if(op==3)//除法
                fprintf(NEWASSEM,"\tdiv $s0,$s1,$s2\n");
        ...
        //函数调用
        else if(strcmp(newinter[newi].inter0,"push")==0)
        {
            strcpy(parastack[parapc++],newinter[newi].inter1);
        }
        else if(strcmp(newinter[newi].inter0,"call")==0)
        {
            strcpy(token,newinter[newi].inter1);
            if(checkiflog()==2||checkiflog()==3)//是函数
            {
                ...
                {
                    //把fp存到sp+4
                    fprintf(NEWASSEM,"\tsw $fp,16($sp)\n");
                    //把sp存到sp+3
                    fprintf(NEWASSEM,"\tsw $sp,12($sp)\n");
                    //把返回值存到sp+2，把PC返回地址存到sp+1
                    fprintf(NEWASSEM,"\tjal Label_%s\n",token);
                }
                else
                {
                    strcpy(funcname,token);
        ...
        //函数返回
        else if(strcmp(newinter[newi].inter0,"ret")==0)
        {
            //返回值
            strcpy(token,newinter[newi].inter1);
            //printf("%s\n",token);
            if(strcmp(token,"none")!=0){
                if(checkiflog()==0||checkiflog()==1||checkiflog()==4)//常量或变量或参数
                {
                    ...
                    fprintf(NEWASSEM,"\tori $s0,$0,%s\n",token);
                fprintf(NEWASSEM,"\tsw $s0,-8($fp)\n");
            }
            fprintf(NEWASSEM,"\tlw $ra,-12($fp)\n");
            fprintf(NEWASSEM,"\tlw $sp,-4($fp)\n");
            fprintf(NEWASSEM,"\tlw $fp,0($fp)\n");
            fprintf(NEWASSEM,"\tjr $ra\n");
        }
        //Label:
        else if(strcmp(newinter[newi].inter1,":")==0)
        {
            fprintf(NEWASSEM,"%s:\n",newinter[newi].inter0);
        }
        //goto Label
        else if(strcmp(newinter[newi].inter0,"goto")==0)
        {
            fprintf(NEWASSEM,"\tj %s\n",newinter[newi].inter1);
        }
        //条件判断
        else if(strcmp(newinter[newi].inter0,"==")==0||strcmp(newinter[newi].inter0,"!=")==0||
                strcmp(newinter[newi].inter0,"<")==0||strcmp(newinter[newi].inter0,"<=")==0||
                strcmp(newinter[newi].inter0,">")==0||strcmp(newinter[newi].inter0,">=")==0)
        {
            if(strcmp(newinter[newi].inter0,"==")==0) con=0;
            else if(strcmp(newinter[newi].inter0,"!=")==0) con=1;
            else if(strcmp(newinter[newi].inter0,"<")==0) con=2;
            else if(strcmp(newinter[newi].inter0,"<=")==0) con=3;
            else if(strcmp(newinter[newi].inter0,">")==0) con=4;
            else if(strcmp(newinter[newi].inter0,">=")==0) con=5;
            //第一个操作数
            strcpy(token,newinter[newi].inter1);
            if(checkiflog()==0||checkiflog()==1||checkiflog()==4)//常量或变量或参数
            {
            ...
            fprintf(NEWASSEM,"\tsub $s0,$s1,$s2\n");
            if(con==0) // ==
                fprintf(NEWASSEM,"\tbne $s1,$s2,");
            else if(con==1)// !=
                fprintf(NEWASSEM,"\tbeq $s1,$s2,");
            else if(con==2)// <
                fprintf(NEWASSEM,"\tbgez $s0,");
            else if(con==3)// <=
                fprintf(NEWASSEM,"\tbgtz $s0,");
            else if(con==4)// >
                fprintf(NEWASSEM,"\tblez $s0,");
            else if(con==5)// >=
                fprintf(NEWASSEM,"\tbltz $s0,");

            //跳转到Label
            fprintf(NEWASSEM,"%s\n",newinter[++newi].inter1);
        }
        //scanf
        else if(strcmp(newinter[newi].inter0,"scan")==0)
        {
            //标识符
            strcpy(token,newinter[newi].inter1);
            if(checkiflog()==1||checkiflog()==4)//变量或参数
            {
                if(symtab[tabi].type==1)//读入int
                {
                     fprintf(NEWASSEM,"\tli $v0,5\n");
                     fprintf(NEWASSEM,"\tsyscall\n");
                }
                else if(symtab[tabi].type==0)//读入char
                {
                    fprintf(NEWASSEM,"\tli $v0,12\n");
                     fprintf(NEWASSEM,"\tsyscall\n");
                }
                if(symtab[tabi].location==0)//全局变量
                    fprintf(NEWASSEM,"\tsw $v0,%d($0)\n",symtab[tabi].address);
                else//局部变量
                {
                    fprintf(NEWASSEM,"\tsw $v0,%d($fp)\n",4*tncount[defloc]+symtab[tabi].address);
                }

            }
        }
        //printf
        else if(strcmp(newinter[newi].inter0,"print")==0)
        {
            if(newinter[newi].inter1[0]!='\0')//字符串
            {
                ...
                for(mipstri=0;mipstri<mipstrpc;mipstri++)
                {
                    if(strcmp(newinter[newi].inter1,mipstring[mipstri].str)==0)
                    {
                        fprintf(NEWASSEM,"\tla $a0,%s\n",mipstring[mipstri].name);
                        fprintf(NEWASSEM,"\tli $v0,4\n");
                        fprintf(NEWASSEM,"\tsyscall\n");
                        break;
                    }
            ...
            else//表达式
            {
                ...
                {
                    if(symtab[tabi].location==0)//全局常变量
                        fprintf(NEWASSEM,"\tlw $a0,%d($0)\n",symtab[tabi].address);
                    else//局部常变量
                        fprintf(NEWASSEM,"\tlw $a0,%d($fp)\n",4*tncount[defloc]+symtab[tabi].address);
                    if(symtab[tabi].type==1)//打印int
                    {
                         fprintf(NEWASSEM,"\tli $v0,1\n");
                         fprintf(NEWASSEM,"\tsyscall\n");
                    }
                    else if(symtab[tabi].type==0)//打印char
                    {
                         fprintf(NEWASSEM,"\tli $v0,11\n");
                         fprintf(NEWASSEM,"\tsyscall\n");
                    }
                }
                else if(checkiftn()>0)//tn
                ...
                {
                    fprintf(NEWASSEM,"\tori $a0,$0,%s\n",token);
                    fprintf(NEWASSEM,"\tli $v0,1\n");
                    fprintf(NEWASSEM,"\tsyscall\n");
                }
            }
            fprintf(NEWASSEM,"\tla $a0,enter\n",newinter[newi].inter1);
            fprintf(NEWASSEM,"\tli $v0,4\n");
            fprintf(NEWASSEM,"\tsyscall\n");
        }
        fprintf(NEWASSEM,"\n");
    }
    fprintf(NEWASSEM,"Label_end:\n");
}
...
int main()
{
    ...
    NEWINTER = fopen("newinter.txt","w");
    ASSEM = fopen("assem.txt","w");
    NEWASSEM = fopen("newassem.txt","w");
